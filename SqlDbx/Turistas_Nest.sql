...
SELECT 
	...
	tveh.DescTipoVehiculo,
	poliz.DescGrupoTipoPoliza,
	aseg.NombreAsegurado,
	turi.NumPoliza,
	
   Tipo_vehículoII=CASE 
  	WHEN tveh.IdTipoVehiculo IN (5676, 22809) THEN 'Vehiculo Pesado'
  	ELSE 'Otros'
   	END ,
	...
	Prima_Emitida= SUM(turi.PrimaNetaPropiaEmitida)
...
INNER JOIN VW_BI_AuttFactEmision AS turi
ON agentes.perfil=turi.NipPerfilAgente
and turi.Periodo >=202101
   	
left JOIN HDI_DWH.dbo.TB_BI_DimTipoVehiculo  tveh
ON tveh.IdTipoVehiculo= turi.IdTipoVehiculo
	
	
left JOIN HDI_DWH.dbo.TB_BI_DimTipoPoliza poliz
ON poliz.IdTipoPoliza= turi.IdTipoPoliza
