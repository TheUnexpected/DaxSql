    ...
    --, NipPerfilAgente  = axc.NipPerfilAgente
	, CASE WHEN catm.NombreEstado='CIUDAD DE MÉXICO' THEN catm.NombreEstado
		WHEN catm.nombremunicipio IN ('LEON', 'MONTERREY', 'GUADALAJARA','TIJUANA') THEN catm.nombremunicipio
		ELSE 'Resto Pais' END AS Ciudad
	,age.DescCanalComercial
	
	, count(ctz.NumCompletoCotizacion) as Cotizaciones 
	...
	on cv.IdVehiculoInterno =plz.IdVehiculo 
	 
	left join TB_BI_DimPaquete AS t3  
	on   t3.IdPaquete = plz.IdPaquete
	
	LEFT JOIN tb_bi_dimagente AS age
	ON age.NipPerfilAgente=axc.NipPerfilAgente
