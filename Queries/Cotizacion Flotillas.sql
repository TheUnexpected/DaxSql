...
full JOIN (
	...
	FROM (	
		...
		SELECT 
			...
			cot.NumUnidades,	    
		    cot.TipoDocumento,
		    CASE WHEN (emisiones.ot_cotizacion >0 OR cot.EstatusCotizacion = 38448) THEN 1
				ELSE 0 END AS PLZ
		...
		WHERE 
	        ...
	        'Mexico Despachos'
	          ...
	          AND cot.EstatusSuscripcion IN ('Terminado', 'Urgente')
		        AND cot.EstatusValidacion NOT IN ('Rechazada', 'Bloqueo Cuenta')
		        AND cot.EstatusClaveteo NOT IN ('Rechazada')
