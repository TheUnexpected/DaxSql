...
ALTER view [Ventas].[VW_DWH_AutrConservacion_IDRIVING]  
/****************************************************************    
Servidor: DWH.HDI.COM.MX    
Base de datos: HDI_DWH    
Proceso de Negocio: Inteligencia de Ventas/Comercial  
Proceso consume: Emision  
    
    
NOMBRE : VW_Conservacion_IDRIVING   
LineaNegocio: Autos  
...
FROM (  
  ...
  INNER JOIN (  
     ...
     ON emi.NumCompletoCotizacion=tec.NumCompletoCotizacion  
     AND emi.NumDocumento=tec.NumDocumento  
     AND emi.IdPaquete IN (2458, 2459, 2529, 2530, 2607, 2608, 2609, 2610, 4034, 4035) --Modificacion Moncada 10/02/2025 - Agregan paquetes iDriving faltantes
  ...
  WHERE   cast(convert(varchar(6),emi.FechaFinVigencia,112) as int)>= 202104  
